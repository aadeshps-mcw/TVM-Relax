...
def dnnl_conv2d_checker(ctx) -> bool:
...
def _make_sum_pattern(op_name: str, with_relu: bool):
    """conv(+bias) -> add(residual)[-> relu]. Distinct from bias-only fusion:
    this matches a *second* add whose other operand is an external residual
    tensor, not the bias."""
    data = wildcard()
    weight = wildcard()
    bias = wildcard()
    ...
    conv_out = is_op(op_name)(data, weight)
    biased = is_op("relax.add")(conv_out, bias)
    summed = is_op("relax.add")(biased, residual)

    return is_op("relax.nn.relu")(summed) if with_relu else summed
...
def _standalone_patterns() -> list[Pattern]:
    ...
    # ^ NOTE: see caveat below — clip as a standalone op has no preceding conv/matmul,
    # so this needs its own tiny pattern rather than reusing _make_clip_pattern's
    # conv/matmul-rooted shape. Simplest: just is_op("relax.clip")(wildcard(), wildcard(),
    # wildcard()).
...
def _make_fused_variants(prefix: str, op_name: str) -> list[Pattern]:
    ...
    for suffix, act_op in _ACTIVATIONS:
        ...
        _validate_eltwise_op_name(
            act_short
        )  # raises ValueError for gelu/swish until confirmed supported
...
def _dnnl_patterns() -> list[Pattern]:
    ...
    # 1. Base ops (no-fusion fallback)
    # 1. Base ops (no-fusion fallback)
    patterns.append(_op_pattern("dnnl.conv2d", "relax.nn.conv2d", 2, dnnl_conv2d_checker))
    patterns.append(_op_pattern("dnnl.matmul", "relax.matmul", 2))
    patterns.append(_op_pattern("dnnl.layer_norm", "relax.nn.layer_norm", 3))

    # 2. Fused variants (bias / activation / clip), shared across base ops
    for prefix, op_name in [
        ("dnnl.conv2d", "relax.nn.conv2d"),
        ("dnnl.matmul", "relax.matmul"),
    ]:
        patterns.extend(_make_fused_variants(prefix, op_name))

    patterns.append(("dnnl.conv2d_bias_sum", _make_sum_pattern("relax.nn.conv2d", with_relu=False)))
    patterns.append(
        ("dnnl.conv2d_bias_sum_relu", _make_sum_pattern("relax.nn.conv2d", with_relu=True))
    )
    ...
    patterns.append(_op_pattern("dnnl.global_avg_pool2d", "relax.nn.adaptive_avg_pool2d", 1))
...
def rewrite_resnet_downsample(func: relax.Function) -> relax.Function:
    ...
    # Add the required func argument here
    ...
    def transform_module(self, mod, ctx):
        for gv, func in mod.functions.items():
            if isinstance(func, relax.Function):
                # Pass func directly instead of chaining calls
...
def partition_for_dnnl(
    ...
    # 1. Apply the downsample rewrite before any partitioning begins
...
_DNNL_COMPUTE_OPS = {
    ...
    "relax.nn.layer_norm",
    # Standalone ops: for these, the op itself IS what DNNL dispatches on,
    # so a subgraph containing only one of these is not "trivial" the way
    # e.g. a lone leftover bias-add would be. Without these, every
    # standalone composite gets demoted by prune_dnnl_subgraphs even
    # though the composite/codegen promotion worked correctly.
    "relax.abs",
    "relax.exp",
    "relax.log",
    "relax.sqrt",
...
def rewrite_batch_norm(mod: tvm.IRModule) -> tvm.IRModule:
    ...
    survives for _make_batch_norm_pattern() to match against.
    """
    x_pat = wildcard()
    mean_pat = wildcard()
    var_pat = wildcard()
